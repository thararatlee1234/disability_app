...
class PersonWithDisability(models.Model):
    ...
    map_url = models.URLField('ลิงก์ Google Maps', max_length=1000, blank=True)
    photo = models.ImageField('รูปถ่าย', upload_to='photos/', null=True, blank=True)
    notes = models.TextField('หมายเหตุ', blank=True)
    latitude = models.DecimalField('ละติจูด', max_digits=15, decimal_places=10, null=True, blank=True)
    longitude = models.DecimalField('ลองจิจูด', max_digits=15, decimal_places=10, null=True, blank=True)
